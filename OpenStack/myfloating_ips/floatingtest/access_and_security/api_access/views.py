...
from openstack_dashboard.dashboards.floatingtest.access_and_security.api_access \
...
def download_ec2_bundle(request):
    ...
    template = 'floatingtest/access_and_security/api_access/ec2rc.sh.template'
    try:
...
def download_rc_file_v2(request):
    template = 'floatingtest/access_and_security/api_access/openrc_v2.sh.template'
...
def download_rc_file(request):
    template = 'floatingtest/access_and_security/api_access/openrc.sh.template'
    context = _get_openrc_credentials(request)
...
class CredentialsView(forms.ModalFormMixin, views.HorizonTemplateView):
    template_name = 'floatingtest/access_and_security/api_access/credentials.html'
...
class RecreateCredentialsView(forms.ModalFormView):
    form_class = floatingtest_forms.RecreateCredentials
    form_id = "recreate_credentials"
    modal_header = _("Recreate EC2 Credentials")
    template_name = \
        'floatingtest/access_and_security/api_access/recreate_credentials.html'
    submit_label = _("Recreate EC2 Credentials")
    submit_url = reverse_lazy(
        "horizon:floatingtest:access_and_security:api_access:recreate_credentials")
...
def download_juju_env_file(request):
    ...
    template = 'floatingtest/access_and_security/api_access/juju.environments.template'
