...
class MyRyu(app_manager.RyuApp):
    ...
    def send_lldp_packet(self, datapath, port_no, hw_addr):
        ...
        prefix='\x00\x00'
        tlv_chassis_id = lldp.ChassisID(subtype=4, chassis_id=prefix+str(struct.pack(">I", datapath.id)))
    ...
    def handle_lldp(self, datapath, port, pkt_ethernet, pkt_lldp):
        prefix='\x00\x00'
        self.lldp_topo.setdefault(prefix+str(struct.pack(">I", datapath.id)),{})
        port_connect = {}
        self.lldp_topo[prefix+str(struct.pack(">I", datapath.id))].setdefault(port, [pkt_lldp.tlvs[0].chassis_id, pkt_lldp.tlvs[1].port_id])
        print "==============> "
        print datapath.id
        print self.lldp_topo.get(prefix+str(struct.pack(">I", datapath.id)), {})
